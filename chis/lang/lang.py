...
def update_dataframe(df):
    if 'Hebrew,Arabic' in df.index:
        for col in df.columns:
            if df.at['Hebrew,Arabic', col] > 0:
                df.at['Hebrew', col] += df.at['Hebrew,Arabic', col]
                df.at['Arabic', col] += df.at['Hebrew,Arabic', col]
        df.drop('Hebrew,Arabic', inplace=True)
# Function to process each group
def process_group(group):
    response=[]
    for i in range(len(group)):
        if type(group.iloc[i][ATT])==str and "," in group.iloc[i][ATT]:
            dat=json.loads(group.iloc[i][ATT])
            response.append(dat["choices"][0]+","+dat["choices"][1])
