...
def process_group(group):
    ...
    for i in range(len(group)):
        #if not nan, there are animals/persons
        a=not pd.isna(group.iloc[i][ATT])
        b=not pd.isna(group.iloc[i][ATT2])
        response.append(a or b)
    if len(response)==0:
...
columns = ['id', POINT_OF_VIEW, CHOSEN,ATT,ATT2]
